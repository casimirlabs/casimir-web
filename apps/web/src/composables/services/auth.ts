...
export default function useAuth() {
    ...
    async function checkIfPrimaryUserExists(
        ...
        try {
            ...
            const response = await fetch(
                `${apiUrl}/user/check-if-primary-address-exists/${address}`,
                requestOptions
            )
    ...
    async function checkIfSecondaryAddress(
        ...
        try {
            ...
            const response = await fetch(
                `${apiUrl}/user/check-secondary-address/${address}`,
                requestOptions
            )
            const { error, message, data } = await response.json()
            if (error) throw new Error(message)
   ...
   */
    ...
    onMounted(async () => {
        ...
        if (!initializeComposable.value) {
            ...
            const { user } = await (await fetch(`${apiUrl}/user`)).json()
